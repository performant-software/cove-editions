...
export const useEmbeddedTEIAnnotations = (xml?: string) => {
  ...
  useEffect(() => {
    ...
    const annotationLists = Array.from(standoffElements).reduce<
      AnnotationList[]
    >((lists, standoffEl, idx) => {
      const layerId =
        standoffElements.length > 1
          ? `tei_standoff_${idx + 1}`
          : "tei_standoff";
  ...
  const annotations = useMemo(
    () =>
      annotationLists.reduce<TextAnnotation[]>(
        (all, list) => [...all, ...list.annotations],
        []
      ),
    [annotationLists]
  );
